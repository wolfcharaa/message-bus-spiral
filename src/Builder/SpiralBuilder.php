...
class SpiralBuilder implements HandlerBuilderInterface
{
    ...
    {
        /** @var TargetInterface<Target> $spiralTarget */
        $spiralTarget = match (true) {
        ...
        if (\count($this->middleware) === 0) {
            return new SpiralHandler($this->pipelineBuilder->build($this->autowireHandler), $spiralTarget);
        }

        return new HandlerWithMiddleware(
            new SpiralHandler($this->pipelineBuilder->build($this->autowireHandler), $spiralTarget),
            array_map(function (Middleware|InterceptorInterface $middleware): Middleware {
